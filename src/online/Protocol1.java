...
public class Protocol1 {
	...
	public static<T> T[] compute(CompEnv<T> gen, T[][] aliceBF, T[][] bobBF) {
		...
		for(int i = 0; i < aliceBF.length; ++i) {
			...
			tmp = lib.absolute(tmp);
			res[i] = lib.multiply(tmp, tmp);
		}
		return add(lib, res);
	}
	...
	public static class Generator<T> extends GenRunnable<T> {
		...
		public void prepareInput(CompEnv<T> gen) throws Exception {
			...
			sketch = new SketchForProtocol1[list.size()];
			sketch[0] = new SketchForProtocol1(SIZE, MEDIAN);
			for(int i = 1; i < list.size(); ++i) {
				sketch[i] = new SketchForProtocol1(SIZE, MEDIAN);
				sketch[i].sks = sketch[0].sks;
			}
			gen.channel.writeInt(numberOfFile);
			for(int i = 0; i < sketch[0].sks.length; ++i)
				gen.channel.writeLong(sketch[0].sks[i]);
			gen.channel.flush();
			...
			for(int listindex = 0; listindex < list.size(); ++listindex) {
				SketchForProtocol1Callable scon = new SketchForProtocol1Callable();
	...
	public static class Evaluator<T> extends EvaRunnable<T> {
		...
		public void prepareInput(CompEnv<T> gen) throws Exception {
			...
			sketch = new SketchForProtocol1(SIZE, MEDIAN);
			numberOfFile = gen.channel.readInt();
			for(int i = 0; i < sketch.sks.length; ++i)
				sketch.sks[i] = gen.channel.readLong();
